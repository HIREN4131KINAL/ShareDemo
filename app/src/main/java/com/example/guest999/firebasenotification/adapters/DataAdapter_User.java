...
import com.example.guest999.firebasenotification.utilis.Delete_user_data;
...
/**
 * Created by Harshad on 25-10-2016 at 09:46 AM and section of downloding image or file by Hiren.
 */
public class DataAdapter_User extends RecyclerView.Adapter implements DownloadCallBack {
	...
	private Delete_user_data delete_data;

	public DataAdapter_User(Context applicationContext, ArrayList<HashMap<String, String>> hello, SwipyRefreshLayout swipeRefreshLayout) {
	...
	public void onBindViewHolder(final RecyclerView.ViewHolder holder, final int position) {
		...
		if (holder instanceof FilePick) {
			...
			final String pdfname = file_paths.get(position).get(Config.TAG_DATA).replace(Config.INTERNAL_IMAGE_PATH_URI, "");
...
//						File myFile = new File(extStore.getAbsolutePath() + "/P L Shah/P L Shah File/" + file.temp.getText().toString());
			...
			try {
				...
				if (!file_paths.get(position).get(Config.KEY_PHONE).equals(PhoneFromDevice)) {
					...
					image.outgoing_layout_bubble.setGravity(Gravity.START | Gravity.BOTTOM);
					image.linearLayout.setGravity(Gravity.START | Gravity.BOTTOM);
					boolean tabletSize = context.getResources().getBoolean(R.bool.tablet);
					if (tabletSize) {
						// do something
					...
					image.outgoing_layout_bubble.setGravity(Gravity.END | Gravity.BOTTOM);
					image.linearLayout.setGravity(Gravity.END | Gravity.BOTTOM);
					boolean tabletSize = context.getResources().getBoolean(R.bool.tablet);
					if (tabletSize) {
						// do something
			...
			image.score.setOnLongClickListener(new View.OnLongClickListener() {
				@Override
				public boolean onLongClick(View v) {
		...
		} else if (holder instanceof DataAdapter_User.TextSend) {
			try {
				final String Contact_name = file_paths.get(position).get(Config.TAG_DATA);
	...
	private void Delet_data(int position, int size) {
		...
		if (CheckConnection.ni != null) {
			...
			delete_data = new Delete_user_data(context, adid, swipeRefreshLayout);
